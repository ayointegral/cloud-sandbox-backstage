...
test.describe('E2E Tests - Catalog Browsing', () => {
  test('User can browse catalog and view entities', async ({ page }) => {
    ...
    const entityList = page.locator(
      'table, [class*="entity"], [class*="catalog"]',
    );
  ...
  test('User can filter catalog by kind', async ({ page }) => {
    ...
    const _kindFilter = page
      .locator('text=/component|service|api|system/i')
      .first();

    // Click on Components filter if available
    const componentsTab = page
      .getByRole('tab', { name: /component/i })
  ...
  test('User can view component details', async ({ page }) => {
    ...
    if (await componentLink.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const aboutCard = page
        .locator('text=/about|overview|description/i')
        .first();
...
test.describe('E2E Tests - TechDocs Navigation', () => {
  ...
  test('User can view documentation for terraform-modules', async ({
    page,
  }) => {
  ...
  test('User can navigate TechDocs sidebar', async ({ page }) => {
    ...
    const nav = page
      .locator('nav, [class*="sidebar"], [class*="navigation"]')
      .first();
...
test.describe('E2E Tests - Scaffolder/Create', () => {
  test('User can browse templates', async ({ page }) => {
    ...
    const templates = page
      .locator('[class*="template"], [class*="card"]')
      .first();
  ...
  test('User can start template wizard', async ({ page }) => {
    ...
    await page.goto(
      `${BASE_URL}/create/templates/default/terraform-module-template`,
    );
    const signedIn = await handleSignIn(page);
    await waitForPageReady(page);
    ...
    // Should see the form
    const form = page
      .locator('form, [class*="wizard"], [class*="stepper"]')
      .first();
...
test.describe('E2E Tests - Search Functionality', () => {
  ...
  test('User can use search from header', async ({ page }) => {
    ...
    const headerSearch = page
      .locator('header input[type="search"], header [class*="search"]')
      .first();
...
test.describe('E2E Tests - Settings Page', () => {
  ...
  test('User can view profile in settings', async ({ page }) => {
    ...
    const profileSection = page
      .locator('text=/profile|user|account|general/i')
      .first();
    const settingsContent = page
      .locator('main, [class*="settings"], [class*="content"]')
      .first();

    // Check either profile section or settings content is visible
    const profileVisible = await profileSection
...
test.describe('E2E Tests - Error Handling', () => {
  ...
  test('Invalid entity shows appropriate message', async ({ page }) => {
    await page.goto(
      `${BASE_URL}/catalog/default/component/nonexistent-component-xyz`,
    );
    const signedIn = await handleSignIn(page);
    await waitForPageReady(page);
    ...
    const errorMessage = page
      .locator('text=/not found|error|does not exist|warning/i')
      .first();
    const _hasError = await errorMessage
      .isVisible({ timeout: 5000 })
      .catch(() => false);
...
test.describe('E2E Tests - Responsiveness', () => {
  ...
  test('Page renders correctly on tablet', async ({ page }) => {
    ...
    await page.goto(`${BASE_URL}/catalog`);
    const _signedIn = await handleSignIn(page);
    await waitForPageReady(page);

    // Page should still be functional
    const content = page
      .locator('main, [class*="content"], article, body')
      .first();
