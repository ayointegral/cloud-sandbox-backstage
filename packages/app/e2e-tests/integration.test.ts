...
test.describe('Integration Tests - Catalog Entity Operations', () => {
  test('Catalog returns consistent data across endpoints', async ({
    request,
  }) => {
    // Get all components
  ...
  test('Catalog by-refs returns requested entities', async ({ request }) => {
    ...
    const entitiesResponse = await request.get(
      `${API_BASE}/catalog/entities?limit=3`,
    );
    ...
    if (entities.length > 0) {
      ...
      // Fetch by refs
      const byRefsResponse = await request.post(
  ...
  test('Catalog facets match entity counts', async ({ request }) => {
    ...
    const componentFacet = facetsData.facets.kind.find(
      (f: any) => f.value.toLowerCase() === 'component',
    );
...
test.describe('Integration Tests - Scaffolder Templates', () => {
  test('All templates are listed and have required fields', async ({
    request,
  }) => {
    // Get all templates
  ...
  test('Scaffolder actions are available', async ({ request }) => {
    const actionsResponse = await request.get(
      `${API_BASE}/scaffolder/v2/actions`,
    );
...
test.describe('Integration Tests - Search Integration', () => {
  test('Search returns results from catalog', async ({ request }) => {
    ...
    const entitiesResponse = await request.get(
      `${API_BASE}/catalog/entities?limit=5`,
    );
    const entities = await entitiesResponse.json();
...
test.describe('Integration Tests - Permission System', () => {
  test('Permission API handles multiple permission checks', async ({
    request,
  }) => {
...
test.describe('Integration Tests - Entity Relationships', () => {
  test('System contains related components', async ({ request }) => {
    ...
    if (systems.length > 0) {
      ...
      const _systemRef = `system:${system.metadata.namespace || 'default'}/${
