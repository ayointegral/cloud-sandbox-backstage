...
test.describe('Smoke Tests - Frontend Routes', () => {
  ...
  for (const route of routes) {
    test(`Route: ${route.name} (${route.path})`, async ({ page }) => {
      ...
      const _errorText = await page
        .locator('text=/error|500|404/i')
        .first()
        .isVisible()
        .catch(() => false);
...
test.describe('Smoke Tests - Catalog API', () => {
  ...
  test('GET /api/catalog/entities (filter by kind=Component)', async ({
    request,
  }) => {
    const response = await request.get(
      `${API_BASE}/catalog/entities?filter=kind=component&limit=10`,
    );
    expect(response.status()).toBe(200);
    const data = await response.json();
    expect(Array.isArray(data)).toBe(true);
  ...
  test('GET /api/catalog/entities (filter by kind=Template)', async ({
    request,
  }) => {
    const response = await request.get(
      `${API_BASE}/catalog/entities?filter=kind=template&limit=10`,
    );
    expect(response.status()).toBe(200);
    const data = await response.json();
    expect(Array.isArray(data)).toBe(true);
  ...
  test('GET /api/catalog/entities (filter by kind=System)', async ({
    request,
  }) => {
    const response = await request.get(
      `${API_BASE}/catalog/entities?filter=kind=system&limit=10`,
    );
    expect(response.status()).toBe(200);
    const data = await response.json();
    expect(Array.isArray(data)).toBe(true);
  ...
  test('GET /api/catalog/entities (filter by kind=Domain)', async ({
    request,
  }) => {
    const response = await request.get(
      `${API_BASE}/catalog/entities?filter=kind=domain&limit=10`,
    );
    expect(response.status()).toBe(200);
  });

  test('GET /api/catalog/entities (filter by kind=Group)', async ({
    request,
  }) => {
    const response = await request.get(
      `${API_BASE}/catalog/entities?filter=kind=group&limit=10`,
    );
    expect(response.status()).toBe(200);
  });

  test('GET /api/catalog/entities (filter by kind=User)', async ({
    request,
  }) => {
    const response = await request.get(
      `${API_BASE}/catalog/entities?filter=kind=user&limit=10`,
    );
    expect(response.status()).toBe(200);
  });

  test('GET /api/catalog/entities (filter by kind=API)', async ({
    request,
  }) => {
    const response = await request.get(
      `${API_BASE}/catalog/entities?filter=kind=api&limit=10`,
    );
    expect(response.status()).toBe(200);
  });
  ...
  test('GET /api/catalog/entity-facets', async ({ request }) => {
    const response = await request.get(
      `${API_BASE}/catalog/entity-facets?facet=kind`,
    );
    expect(response.status()).toBe(200);
  });
  ...
  test('POST /api/catalog/entities/by-refs', async ({ request }) => {
    const response = await request.post(
      `${API_BASE}/catalog/entities/by-refs`,
      {
        data: { entityRefs: [] },
...
test.describe('Smoke Tests - TechDocs API', () => {
  ...
  test('GET /api/techdocs/metadata/techdocs for terraform-modules', async ({
    request,
  }) => {
    const response = await request.get(
  ...
  test('GET /api/techdocs/metadata/entity for terraform-modules', async ({
    request,
  }) => {
    const response = await request.get(
      `${API_BASE}/techdocs/metadata/entity/default/component/terraform-modules`,
    );
    expect([200, 404]).toContain(response.status());
  });
});
...
test.describe('Smoke Tests - Search API', () => {
  ...
  test('GET /api/search/query (with term)', async ({ request }) => {
    const response = await request.get(
      `${API_BASE}/search/query?term=terraform`,
    );
    expect(response.status()).toBe(200);
    const data = await response.json();
    expect(data).toHaveProperty('results');
...
test.describe('Smoke Tests - Auth API', () => {
  test('GET /api/auth/github/start is available', async ({ request }) => {
    ...
    const response = await request
      .get(`${API_BASE}/auth/github/start?env=development`, {
        maxRedirects: 0,
      })
      .catch(e => e);
